...
(function() {
  ...
  /**
   *
   * The router must be initiated and
   * Backbone.history.start(); must be called so Backbone starts monitoring the routes.
   *
   * No logic or as little possible should be done within the router.
   * It is better to trigger event from the router and delegate.
   */
  ...
  App.Views.Appointement = Backbone.View.extend({
    initialize: function() {
      vent.on('appointment:show',this.show,this); //other.on(event, callback, object)
      //vent.listenTo(this.model,'appointment:show',this.show); //object.listenTo(other, event, callback)
    },
    show:function(id){
      console.log('App.Views.Appointment - show - showing the appointement with the id ',id);
    }
  });
  ...
  new App.Views.Appointement({model:personBackbone});
